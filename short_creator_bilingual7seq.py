...
class Config:
    ...
    # Timeline — 7-segment layout (Hook → Img1 → DanceRxn → Img2 → Img3 → Summary → Outro)
    DUR_HOOK: float = 3.0       # Hook: big text + 1-beat dance cut
    DUR_IMG1: float = 13.0      # Ảnh 1 ken-burns + CC
    DUR_DANCE: float = 4.0      # Dance reaction segment
    DUR_IMG2: float = 13.0      # Ảnh 2 wiggle + keyword pop
    DUR_IMG3: float = 13.0      # Ảnh 3 ken-burns + CC
    DUR_SUMMARY: float = 6.0    # 3-bullet summary text
    DUR_OUTRO: float = 7.0      # CTA + dance cut + credit
...
class VideoCreator:
    ...
    # ── NEW 7-segment builders ────────────────────────────────────────

    def _build_hook(self, clip_files: List[str], duration: float, fps: int,
                    amp_array: np.ndarray, t_offset: float) -> "VideoClip":
        """0–3 s  Hook: black bg + big wiggle text + optional 1-beat dance cut."""
        cfg = self.config
        w, h = cfg.OUTPUT_RESOLUTION
        font_size = max(80, int(w * 0.08))

        def hook_frame(t):
            img = Image.new("RGBA", (w, h), (8, 8, 8, 255))
            draw = ImageDraw.Draw(img)
            f = self._load_font(font_size)
            label = cfg.INTRO_LABEL
            # wiggle amplitude driven by amp_array
            gi = min(int((t + t_offset) * fps), len(amp_array) - 1)
            amp = cfg.WIGGLE_AMP_MIN_PX + float(amp_array[gi]) * (
                cfg.WIGGLE_AMP_MAX_PX - cfg.WIGGLE_AMP_MIN_PX
            )
            dx = int(amp * np.sin(2 * np.pi * 3.0 * t + 0.0))
            dy = int(amp * 0.4 * np.cos(2 * np.pi * 3.0 * t + 1.0))
            ...
            for ox in range(-3, 4):
                for oy in range(-3, 4):
                    if ox or oy:
                        draw.text((x + ox, y + oy), label, font=f,
                                  fill=(0, 0, 0, 180))
            draw.text((x, y), label, font=f, fill=(255, 200, 0, 255))

            # sub-label
            sf = self._load_font(max(32, int(w * 0.033)))
            sub = "📰 59s" if cfg.LANG == "en" else "📰 59 giây"
            bb2 = draw.textbbox((0, 0), sub, font=sf)
            sx = (w - (bb2[2] - bb2[0])) // 2
            sy = y + th2 + 24
            draw.text((sx, sy), sub, font=sf, fill=(210, 210, 210, 220))
            return np.array(img)

        base = _make_video_clip(hook_frame, duration)

        # overlay 1-beat dance cut (first 1.5 s of a random clip)
        if clip_files:
            try:
                cf = random.choice(clip_files)
                dc = VideoFileClip(cf)
                dc = self._resize_clip_to_shorts(dc)
                cut_dur = min(1.5, dc.duration * 0.5)
                start = random.uniform(0, max(0, dc.duration - cut_dur - 0.1))
                dc = _c_subclip(dc, start, start + cut_dur)
                # place at t=1.0 in hook
                dance_start = min(1.0, duration - cut_dur - 0.05)
                dc = _c_duration(dc, cut_dur)
                # build composite: base for [0, dance_start], then dance, then base again
                pre  = _c_subclip(base, 0, dance_start)
                post_start = dance_start + cut_dur
                post = (_c_subclip(base, post_start, duration)
                        if post_start < duration else None)
                parts = [pre, dc] + ([post] if post else [])
                final_hook = concatenate_videoclips(parts, method="compose")
                final_hook = _c_duration(final_hook, duration)
                dc.close()
                self._add_logo([final_hook], duration)
                return self._composite([final_hook], duration)
            except Exception as e:
                logger.warning(f"Hook dance cut failed: {e}")

        self._add_logo([base], duration)
        return self._composite([base], duration)

    def _build_img1(self, img: Image.Image, words: list, timings: list,
                    t_offset: float, duration: float, fps: int) -> "VideoClip":
        """3–16 s  Ảnh 1 ken-burns + CC (like MAIN but longer, no BREAKING title)."""
        bg = self._ken_burns_clip(img, duration, zoom_in=True)
        cc = self._make_cc_overlay(words, timings, t_offset, duration)
        layers = [bg, cc]
        self._add_logo(layers, duration)
    ...
    def _build_dance_rxn(self, clip_files: List[str], duration: float,
                         fps: int, amp_array: np.ndarray,
                         t_offset: float, reaction_text: str = "") -> "VideoClip":
        """16–20 s  Dance reaction: dance jumpcut + small overlay text."""
        cfg = self.config
        w, h = cfg.OUTPUT_RESOLUTION
        base = self._build_jumpcut_sequence(clip_files, duration, n_segments=1)
        base = _c_duration(base, duration)
        base = self._apply_beat_zoom_pulse(base, amp_array, fps, t_offset)
        ...
        if reaction_text:
            def rxn_frame(t):
                img = Image.new("RGBA", (w, h), (0, 0, 0, 0))
                draw = ImageDraw.Draw(img)
                f = self._load_font(max(36, int(w * 0.036)))
                bb = draw.textbbox((0, 0), reaction_text, font=f)
                tw2, th2 = bb[2] - bb[0], bb[3] - bb[1]
                bx = (w - tw2) // 2
                by = int(h * 0.82)
                draw.rounded_rectangle(
                    [bx - 18, by - 12, bx + tw2 + 18, by + th2 + 12],
                    radius=14, fill=(0, 0, 0, 190)
                )
                draw.text((bx, by), reaction_text, font=f, fill=(255, 221, 0, 255))
                return np.array(img)

            overlay = _make_video_clip(rxn_frame, duration)
            layers = [base, overlay]
        else:
            layers = [base]
    ...
    def _build_img2(self, img: Image.Image, words: list, timings: list,
                    t_offset: float, duration: float, fps: int,
                    amp_array: np.ndarray) -> "VideoClip":
        """20–33 s  Ảnh 2 full-frame wiggle + keyword pop CC."""
        cfg = self.config
        w, h = cfg.OUTPUT_RESOLUTION
        full = self._cover_crop(img.convert("RGBA"), w, h)
        wiggle = self._make_wiggle_clip(full, duration, fps, amp_array, t_offset, 0, 0)
        flash  = self._flash_overlay(duration)
        cc     = self._make_cc_overlay(words, timings, t_offset, duration)
        layers = [wiggle, flash, cc]
        self._add_logo(layers, duration)
        return self._composite(layers, duration)

    def _build_img3(self, img: Image.Image, words: list, timings: list,
                    t_offset: float, duration: float, fps: int) -> "VideoClip":
        """33–46 s  Ảnh 3 ken-burns zoom-out + CC."""
        bg   = self._ken_burns_clip(img, duration, zoom_in=False, max_zoom=1.12)
        dark = self._dark_overlay(duration, alpha=90)
        cc   = self._make_cc_overlay(words, timings, t_offset, duration)
        layers = [bg, dark, cc]
        self._add_logo(layers, duration)
        return self._composite(layers, duration)
    ...
    async def create_compiled_short(
        ...
        # 4. Timeline offsets — 7-segment layout matching txt spec:
        #    Hook(3s) → Ảnh1(13s) → Dance reaction(4s) → Ảnh2(13s) → Ảnh3(13s) → Summary(6s) → Outro(7s)
        t_hook    = 0.0
        t_img1    = t_hook    + cfg.DUR_HOOK    #  3.0 s
        t_dance   = t_img1    + cfg.DUR_IMG1    # 16.0 s
        t_img2    = t_dance   + cfg.DUR_DANCE   # 20.0 s
        t_img3    = t_img2    + cfg.DUR_IMG2    # 33.0 s
        t_summary = t_img3    + cfg.DUR_IMG3    # 46.0 s
        t_outro   = t_summary + cfg.DUR_SUMMARY # 52.0 s
        ...
        # 6. Build 7 segments
        reaction_text = ("What's the impact? 👀" if cfg.LANG == "en"
                         else "Tác động là gì? 👀")

        logger.info(f"[{lang_tag}] [0–3s]   HOOK...")
        seg_hook = self._build_hook(clip_files, cfg.DUR_HOOK, fps, amp_array, t_hook)

        logger.info(f"[{lang_tag}] [3–16s]  IMG1...")
        seg_img1 = self._build_img1(
            main_img, all_words, word_timings, t_img1, cfg.DUR_IMG1, fps)

        logger.info(f"[{lang_tag}] [16–20s] DANCE REACTION...")
        if clip_files:
            seg_dance = self._build_dance_rxn(
                clip_files, cfg.DUR_DANCE, fps, amp_array, t_dance, reaction_text)
        else:
            seg_dance = self._build_flash(
                main_img, all_words, word_timings, t_dance, cfg.DUR_DANCE, fps, amp_array)

        logger.info(f"[{lang_tag}] [20–33s] IMG2...")
        seg_img2 = self._build_img2(
            flash_img, all_words, word_timings, t_img2, cfg.DUR_IMG2, fps, amp_array)

        logger.info(f"[{lang_tag}] [33–46s] IMG3...")
        seg_img3 = self._build_img3(
            summary_img, all_words, word_timings, t_img3, cfg.DUR_IMG3, fps)

        logger.info(f"[{lang_tag}] [46–52s] SUMMARY...")
        seg_summary = self._build_summary(
            summary_img, all_words, word_timings, t_summary, cfg.DUR_SUMMARY, fps)

        logger.info(f"[{lang_tag}] [52–59s] OUTRO...")
        seg_outro = self._build_outro(
            main_img, cfg.TG_CHANNEL_NAME, all_words, word_timings, t_outro, cfg.DUR_OUTRO, fps)

        # 7. Concatenate
        segments = [seg_hook, seg_img1, seg_dance, seg_img2, seg_img3, seg_summary, seg_outro]
...
def _shared_overrides() -> dict:
    return dict(
        ...
        DUR_HOOK=float(os.getenv("DUR_HOOK", "3.0")),
        DUR_IMG1=float(os.getenv("DUR_IMG1", "13.0")),
        DUR_DANCE=float(os.getenv("DUR_DANCE", "4.0")),
        DUR_IMG2=float(os.getenv("DUR_IMG2", "13.0")),
        DUR_IMG3=float(os.getenv("DUR_IMG3", "13.0")),
        DUR_SUMMARY=float(os.getenv("DUR_SUMMARY", "6.0")),
        DUR_OUTRO=float(os.getenv("DUR_OUTRO", "7.0")),
