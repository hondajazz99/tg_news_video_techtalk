# short_creator_bilingual7seq7.py
#
# Bilingual upgrade of short_creator_7_.py
#
# ARCHITECTURE:
...
# Full Unicode emoji pattern — covers:
#   • All standard emoji blocks (Emoticons, Misc Symbols, Transport, Supplemental, etc.)
#   • Regional Indicator pairs (🇻🇳 🇺🇸 ...) — U+1F1E0–U+1F1FF
#   • Keycap sequences  e.g. 1️⃣ (digit + U+FE0F + U+20E3)
#   • ZWJ sequences     e.g. 👨‍💻 (multiple emoji joined by U+200D)
#   • Variation selector U+FE0F (emoji presentation selector — must stay
#     attached to the preceding base codepoint, not fragment into its own run)
#   • Skin-tone modifiers U+1F3FB–U+1F3FF
#
# Strategy: match the longest possible run that includes ZWJ + VS-16 + modifier
# so multi-codepoint sequences are never fragmented into invisible stray glyphs.
_EMOJI_RUN_RE = _re.compile(
    r"(?:"
    # ZWJ sequence or modifier/selector chain — greedy, consumes the whole unit
    r"(?:[\U0001F1E0-\U0001F1FF]{2})"           # Regional Indicator pairs (flags)
    r"|(?:\d\uFE0F\u20E3)"                       # Keycap: digit + VS-16 + combining enclosing
    r"|(?:#\uFE0F\u20E3)"                        # Keycap: # sign
    r"|(?:\*\uFE0F\u20E3)"                       # Keycap: * sign
    r"|(?:"                                      # General emoji + optional modifiers/ZWJ chains
        ...
        r"(?:\u200D"                              #  Optional ZWJ + following emoji (chain)
...
def _wrap_words_by_width(draw: ImageDraw.ImageDraw, words: list, font: ImageFont.FreeTypeFont,
                          max_width: int, space_w: Optional[int] = None) -> list:
    """Greedy word-wrap that breaks lines by *pixel width* instead of a fixed
    word count, so long words / Vietnamese diacritics / mixed emoji never run
    past max_width. Falls back to splitting an oversized single word by
    characters so it still never overflows."""
    if space_w is None:
        space_w = _measure_mixed_width(draw, " ", font) or int(font.size * 0.3)
    lines, cur, cur_w = [], [], 0
    for word in words:
        ww = _measure_mixed_width(draw, word, font)
        if ww > max_width and not cur:
            # Single word wider than the line on its own: hard-break by chars.
            piece = ""
            piece_w = 0
            for ch in word:
                cw = _measure_mixed_width(draw, ch, font)
                if piece and piece_w + cw > max_width:
                    lines.append([piece])
                    piece, piece_w = ch, cw
                else:
                    piece += ch
...
def _autofit_lines(draw: ImageDraw.ImageDraw, load_font_fn, text: str, start_size: int,
                    max_width: int, max_height: int, min_size: int = 22,
                    line_spacing: int = 18) -> Tuple[list, ImageFont.FreeTypeFont, int]:
    """Shrinks font size until the wrapped text fits within (max_width,
    max_height) on the 1080x1920 canvas. Returns (lines, font, font_size).
    Guarantees no horizontal overflow (wrap is width-based) and no vertical
    overflow/clipping (shrinks until the block fits, down to min_size)."""
    words = text.split()
    size = start_size
    while size >= min_size:
        f = load_font_fn(size)
        lines = _wrap_words_by_width(draw, words, f, max_width)
        line_h = size + line_spacing
        total_h = line_h * len(lines)
        if total_h <= max_height:
            return lines, f, size
        size -= 4
    f = load_font_fn(min_size)
    lines = _wrap_words_by_width(draw, words, f, max_width)
    return lines, f, min_size
...
class TelegramClient:
    ...
    def __init__(self, token: str, cache_file: str = ".telegram_photo_cache.json"):
        ...
        self._seen_update_ids = set(self._cache.get("seen_update_ids", []))
        self._raw_updates: Optional[list] = None
        self._pending_max_update_id: Optional[int] = None
    ...
    def _fetch_batch(self) -> list:
        """
        Fetch the pending getUpdates batch ONCE per process run and cache it
        in memory (self._raw_updates). getUpdates/offset is global to the bot
        token and returns updates for ALL channels mixed together — if each
        language pipeline called getUpdates independently, the first call
        would advance the offset past the *entire* batch (including the
        other channel's posts), permanently dropping any of that other
        channel's updates before its own get_latest_images() call ever ran.
        Fetching once and filtering per-channel in memory avoids that.
        """
        if self._raw_updates is not None:
            return self._raw_updates
        ...
        logger.info(f"Updates received (all channels): {len(all_updates)}")
        self._raw_updates = all_updates

        # Track the highest update_id seen so we can commit the offset once,
        # after every channel has had a chance to process this batch.
        self._pending_max_update_id = max(
            [self._cache.get("last_update_id", 0)] + [u.get("update_id", 0) for u in all_updates]
        )
        return self._raw_updates

    def commit(self):
        """
        Advance the persisted offset/cache. Call this ONCE per run, after
        every channel's get_latest_images() has processed the shared batch
        fetched by _fetch_batch(). Committing per-channel (the old behavior)
        is what caused cross-channel updates to be silently dropped.
        """
        if self._pending_max_update_id is not None:
            self._cache["last_update_id"] = self._pending_max_update_id
        self._cache["seen_update_ids"] = list(self._seen_update_ids)
        self._save_cache()
    ...
    def get_latest_images(
        ...
        try:
            seen_update_ids = self._seen_update_ids
            all_updates = self._fetch_batch()
            ...
            # NOTE: offset/seen-id persistence is intentionally NOT done here.
            # seen_update_ids is mutated in place (self._seen_update_ids), so
            # it's already up to date in memory. The offset itself is only
            # written to disk by commit(), which the caller must invoke once
            # after ALL channels have processed this shared batch — otherwise
            # a channel that runs later would have its updates silently
            # skipped past by an offset the first channel already advanced.
...
class VideoCreator:
    ...
    def _make_title_frame(self, t: float, title_text: str) -> np.ndarray:
        ...
        safe_w = int(w * 0.88)
        safe_h = int(h * 0.22)
        lines, f, used_size = _autofit_lines(
            draw, self._load_font, title_text, 74, safe_w, safe_h, min_size=34
        )
        line_h = used_size + 18
        y0 = int(h * 0.20)
    ...
    def _make_pop_text_frame(self, t: float, text: str, font_size: int = 64,
        ...
        Auto-shrinks and wraps by pixel width so it always stays inside the
        1080x1920 safe area — no overflow, no clipped/cut-off characters."""
        w, h = self.config.OUTPUT_RESOLUTION
        img  = Image.new("RGBA", (w, h), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        ...
        safe_w = int(w * 0.88)          # horizontal safe margin
        safe_h = int(h * 0.30)          # vertical budget for this text block
        target_size = max(22, int(font_size * scale))
        lines, f, used_size = _autofit_lines(
            draw, self._load_font, text, target_size, safe_w, safe_h, min_size=26
        )

        line_h = used_size + 18
        total_h = line_h * len(lines)
        y0 = int(h * y_ratio) - total_h // 2
        # Clamp so the block never starts above 0 or runs past the bottom edge.
        y0 = max(int(h * 0.04), min(y0, h - total_h - int(h * 0.04)))

        if box:
            max_lw = max((_measure_mixed_width(draw, " ".join(l), f) for l in lines), default=0)
            bw = min(max_lw + 60, w - 24)
            bx = (w - bw) // 2
            draw.rounded_rectangle(
    ...
    def _make_summary_lines_frame(self, t: float, lines: List[str],
        ...
        Each line is auto-fit (wrapped + shrunk if needed) to the safe width
        so long captions never overflow or get clipped on the 9:16 canvas."""
        w, h = self.config.OUTPUT_RESOLUTION
        img  = Image.new("RGBA", (w, h), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        safe_w = int(w * 0.88)
        ...
        # Pre-fit every line so each block height is known up front, then
        # stack them top-to-bottom inside the vertical safe zone.
        fitted = []
        for ls in lines:
            wraps, f, size = _autofit_lines(
                draw, self._load_font, ls, 52, safe_w, int(h * 0.12), min_size=30
            )
            fitted.append((wraps, f, size))

        y_top = int(h * 0.38)
        y_bottom_limit = int(h * 0.92)
        y = y_top
        for i in range(active_idx + 1):
            wraps, f, size = fitted[i]
            block_line_h = size + 14
            block_h = block_line_h * len(wraps)
            if i < active_idx:
                y += block_h + 24
                continue
            fade = min(max((t - i * line_duration) / 0.2, 0.0), 1.0)
            alpha = int(255 * fade)
            dx = wiggle_dx
            by = min(y, y_bottom_limit - block_h)
            max_lw = max((_measure_mixed_width(draw, " ".join(wl), f) for wl in wraps), default=0)
            bx = (w - max_lw) // 2 + dx
            draw.rounded_rectangle(
                [bx - 24, by - 12, bx + max_lw + 24, by + block_h + 12],
                radius=16, fill=(0, 0, 0, int(150 * fade))
            )
            for li, wl in enumerate(wraps):
                ls2 = " ".join(wl)
                tw2 = _measure_mixed_width(draw, ls2, f)
                lx = (w - tw2) // 2 + dx
                ly = by + li * block_line_h
                _draw_mixed_text(draw, img, (lx, ly), ls2, f, (255, 221, 0, alpha))
        return np.array(img)
    ...
    async def _generate_continuous_tts(
        ...
        def strip_emojis(s: str) -> str:
            return _EMOJI_RUN_RE.sub("", s).strip()
    ...
    def _build_dance_reaction(self, clip_files, words, timings, t_offset, duration, fps,
        ...
        # Bigger, brighter, positioned above the CC caption band (y_ratio
        # 0.78) so it never overlaps the burned-in captions and stays large
        # and easy to read.
        text = self._make_pop_text_overlay(reaction_text, duration, font_size=68,
                                           y_ratio=0.62, pop_in=0.2,
                                           color=(255, 221, 0, 255))
        cc = self._make_cc_overlay(words, timings, t_offset, duration)
        layers = [base, text, cc]
        self._add_logo(layers, duration)
    ...
    def _build_cta(self, main_img, channel, clip_files, words, timings, t_offset,
        ...
        cc = self._make_cc_overlay(words, timings, t_offset, duration, y_ratio=0.85)
        layers.append(cc)
        self._add_logo(layers, duration)
        return self._composite(layers, duration)
    ...
    async def create_compiled_short(
        ...
        # 6b. Secondary TTS — voice the dance question + the 2nd CTA line,
        #     since neither is part of the main narration script above.
        extra_audio_clips: List[Tuple[float, Path]] = []  # (start_time, path)

        async def _voice_extra(text: str, target_dur: float, tag: str) -> Optional[Path]:
            if not text.strip():
                return None
            out = Path(f"temp_tts_{cfg.LANG}_{tag}.mp3")
            try:
                await self._generate_continuous_tts(text, target_dur, out)
                return out
            except Exception as e:
                logger.warning(f"[{lang_tag}] Secondary TTS '{tag}' failed: {e}")
                return None

        dance_tts_path = await _voice_extra(dance_text, cfg.DUR_DANCE, "dance")
        if dance_tts_path:
            extra_audio_clips.append((t_dance, dance_tts_path))
        ...
        for start_time, extra_path in extra_audio_clips:
            try:
                extra_clip = AudioFileClip(str(extra_path))
                extra_clip = _c_vol(extra_clip, cfg.TTS_VOL)
                extra_clip = _c_start(extra_clip, start_time)
                audio_tracks.append(extra_clip)
            except Exception as e:
                logger.warning(f"[{lang_tag}] Could not mix secondary TTS '{extra_path}': {e}")
        ...
        for _, extra_path in extra_audio_clips:
...
async def _main():
    try:
        ...
        # Commit the Telegram offset ONCE, after every requested channel has
        # had a chance to pull its posts from the shared batch. Committing
        # earlier (e.g. inside each pipeline call) is what previously caused
        # whichever channel ran second to have its updates silently dropped.
        telegram.commit()
